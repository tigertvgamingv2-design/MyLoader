...
namespace Loader.UI
{
    public partial class Main : Form
    {
        ...
        {
            // Use proper DPI-based auto scaling now that the app declares PerMonitorV2 DPI awareness
            // in app.manifest, so the window renders at the correct physical size on high-DPI displays
            // (e.g. 4K monitors), instead of CEF silently forcing DPI-awareness without any compensation.
        ...
        public Main()
        {
            ...
            // Set up form size & borders first, so the window is always the correct size
            // even if browser initialization fails below.
            SetupForm();
            ...
            {
                MessageBox.Show(
                    "Failed to initialize the embedded browser. Please ensure all required files are present and try reinstalling the application.\n\n" + ex,
